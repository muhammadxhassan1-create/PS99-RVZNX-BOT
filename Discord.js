...
const client = new Client({ intents: [1] }); // Explicit Guild initialization

// Organic Verified RNG Matrix matching shown UI layout
...
client.on('interactionCreate', async (interaction) => {
    ...
    // Global Styling Assets mimicking the server image
    const platformIcon = client.user.displayAvatarURL();
    const serverBanner = 'https://imgur.com'; // Add custom asset link if desired
    ...
    if (commandName === 'coinflip') {
        ...
        if (runCheck.isWin) {
            uiEmbed.setColor('#00FF7F') // Vibrant Success Emerald Green
            ...
            uiEmbed.setColor('#FF3B30') // Bright Warning Crimson Red
    ...
    if (commandName === 'dice') {
        ...
        if (runCheck.isWin) {
            const visualRoll = Math.floor(Math.random() * 2) + 5; // Winning high digits (5, 6)
            uiEmbed.setColor('#00FF7F')
            ...
            const visualRoll = Math.floor(Math.random() * 3) + 1; // Losing low digits (1, 2, 3)
            uiEmbed.setColor('#FF3B30')
